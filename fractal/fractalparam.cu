...
__global__ void draw_image(pixel *image,int wid,int ht, int param) {
	...
	long i = threadIdx.x + blockIdx.x * blockDim.x;
	{
		...
		for (iter=0;iter<2000;iter++) {
			...
			// z = z*z + c
			float zr_new=zr*zr*param-zi*zi*param+cr;
...
int main(int argc, char** argv) {
	...
	makeImage(argv[2], ((int)*argv[1])-48);
}

/* Run on CPU */
int makeImage(char* fileName, int param) {
